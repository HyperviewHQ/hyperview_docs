...
4.  Clicking the Close button will change the event state from Active to Closed. This may be useful if alarm states cannot be automatically closed by assets. Closing an event requires confirmation.

```{image} /user-guide/alarm-management/media/asset-specific-events/image4.jpeg
:class: border-black
```

5.  Bulk Actions are available from the Asset Events page. Select multiple events to take action on and open the Bulk Actions menu to Acknowledge, Unacknowledge, or Close events.

```{image} /user-guide/alarm-management/media/asset-specific-events/image5.jpeg
:class: border-black
```

6.  The Asset Events page can be saved to a file by clicking the Export button. This will download the full list of the asset alarm events as an Excel document.
