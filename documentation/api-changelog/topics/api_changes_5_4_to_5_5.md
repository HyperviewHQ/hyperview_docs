# API Changelog 5.4 to 5.5

## What's New

### `POST` /api/asset/search/sensors

> Returns a list of sensors that meet the requested criteria.


### `POST` /api/asset/search/multiple

> Returns data grouped by search type that meet the requested criteria.
...
* Changed content type : `application/json`
    ...
    * Added property `hits` (array)

        Items (object):

    * Added property `facetDistribution` (object)

    * Added property `processingTimeMs` (integer)

    * Added property `query` (string)

    * Added property `_matchesPosition` (object)

    * Added property `facetStats` (object)
...
* Added property `filteredSensorThresholdIds` (array)
...
Changed: `includeImageBase64String` in `query`
...
* Changed content type : `application/json`
    ...
    * Added property `sensorThresholds` (array)

        Items (object):

        * Property `id` (string)

        * Property `sensorTypeId` (string)

        * Property `assetTypeId` (string)
            ...
            * `blankingPanel`
            * `dcRectifier`
        * Property `sensorTypeDescription` (string)

        * Property `unitId` (string)

        * Property `unit` (string)

        * Property `comparisonType1` (string)

            Enum values:

            * `compareToConstant`
            * `compareToAssetPropertyValue`
            * `compareToAssetPropertyValuePercentage`
        * Property `comparisonOperator1` (string)
            ...
            * `lessThan`
            * `lessThanOrEqualTo`
        * Property `comparisonConstant1` (number)

        * Property `comparisonAssetPropertyKey1` (string)
            ...
            * `internalStatus`
            * `bladeBmcMacAddress`
            * `operatingSystemPlatformId`
            * `operatingSystemSignature`
            * `operatingSystemVendor`
            * `operatingSystemServicePackMajorVersion`
            * `operatingSystemServicePackMinorVersion`
            * `operatingSystemEditionId`
            * `toolsStatus`
            * `linkState`
            * `captureState`
            * `simulatedCableState`
            ...
            * `breakerNumber`
            * `panelNumber`
            * `defaultMainPanelBreakerSize`
            * `defaultBreakerSize`
            * `latitude`
            * `longitude`
            * `controllerSerialNumber`
            * `powerModuleSerialNumber`
            * `fanSerialNumber`
            ...
            * `assetTrackerModuleSerialNumber`
            * `assetTrackerExpectedReportedRackUnits`
            * `assetTrackerRackOffset`
            * `rackDeltaTCalculationOrientation`
            * `mediaType`
            ...
            * `webPortalLaunchUrl`
            * `biosVersion`
            * `powerSupplyOutputRating`
            * `powerSupplyInputRating`
            * `powerSupplyType`
            * `snmpPort`
            * `businessEntityType`
            * `streetAddressType`
        ...
        * Property `comparisonAssetPropertyPercentage1` (number)

        * Property `comparisonType2` (string)

        * Property `comparisonOperator2` (string)

        * Property `comparisonConstant2` (number)

        * Property `comparisonAssetPropertyKey2` (string)

        * Property `comparisonAssetPropertyPercentage2` (number)
        ...
        * Property `severity` (string)
            ...
            * `warning`
            * `critical`
        * Property `thresholdSource` (string)
...
* Changed content type : `application/json`
    ...
    * Changed property `eventType` (string)
        ...
        * `advancedSavedSearchAdded`
        * `advancedGlobalSavedSearchAdded`
        * `advancedSavedSearchDeleted`
        * `advancedGlobalSavedSearchDeleted`
        * `alarmEventPolicySensorThresholdsUpdated`
        * `samlSsoConfigurationAdded`
        * `samlSsoConfigurationAllowUnsolicitedAuthenticationResponseUpdated`
        * `samlSsoConfigurationAuthenticationRequestsSignedUpdated`
        * `samlSsoConfigurationDeleted`
        * `samlSsoConfigurationDisplayNameUpdated`
        * `samlSsoConfigurationIdentityProviderEntityIdUpdated`
        * `samlSsoConfigurationSsoUrlUpdated`
        * `samlSsoRequiredLoginForSelectedDomainUpdated`
        * `samlSsoTokenEncryptionCertificateExpired`
        * `samlSsoTokenEncryptionCertificateGenerated`
        * `samlSsoTokenSigningCertificateUpdated`
